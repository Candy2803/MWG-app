...
function ManageUsers() {
  ...
  useEffect(() => {
    axios
      .get("http://192.168.1.98:5000/api/users")
      .then((response) => {
  ...
  const handleEdit = (id, index) => {
    ...
    axios
      .put(`http://192.168.1.98:5000/api/users/${id}`, updatedUser)
      .then((response) => {
    ...
    Alert.alert(
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: () => {
            axios
              .delete(`http://192.168.1.98:5000/api/users/${id}`)
              .then(() => {
  ...
  const handleImpersonate = (user) => {
    ...
    axios
      .get(`http://192.168.1.98:5000/api/users/loginadmin/${user._id}`)
      .then(() => {
  ...
  const handleApprove = (id) => {
    axios
      .put(`http://192.168.1.98:5000/api/users/${id}/approve`)
      .then((response) => {
        setUsers(
